...
export const BikeCard: React.FC<BikeCardProps> = ({ bike, onClick, onToggleCompare, onLocateDealers, isComparing, isLocating, labels }) => {
  ...
  const renderStars = (rating: number) => {
    return (
      <div className="flex items-center space-x-0.5">
        ...
        <span className="text-xs text-slate-400 ml-1 font-medium">({rating})</span>
      </div>
    );
  };
  ...
  return (
    <div 
      onClick={onClick}
      className={`group relative bg-slate-900 border rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 ease-out hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/20 flex flex-col h-full ${isComparing ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-slate-800 hover:border-blue-500/40'}`}
    >
      {/* Comparison Toggle */}
        ...
        <button
          ...
          className={`flex items-center justify-center w-8 h-8 rounded-full backdrop-blur-md border transition-all duration-300 shadow-lg ${
            isLocating 
              ...
              : 'bg-slate-900/60 hover:bg-indigo-600 border-white/10 hover:border-indigo-500 text-slate-300 hover:text-white'
          }`}
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-full backdrop-blur-md border transition-all duration-300 shadow-lg ${
            isComparing 
              ...
              : 'bg-slate-900/40 border-white/10 text-slate-300 hover:bg-slate-800 hover:border-blue-500/50 hover:text-white'
          }`}
        >
          ...
          <span className="text-xs font-bold tracking-wide">{labels.compare}</span>
        </button>
      </div>
      ...
      <div className="absolute top-4 left-4 z-30">
        <button
          ...
          className="group/btn flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900/60 hover:bg-indigo-600 backdrop-blur-md border border-white/10 hover:border-indigo-500 text-slate-300 hover:text-white transition-all duration-300 shadow-lg"
        >
          {isGeneratingImage ? (
      ...
      <div className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out bg-gradient-to-r from-transparent via-white/5 to-transparent z-20 pointer-events-none"></div>

      {/* Image Header Container */}
      <div className="h-48 relative overflow-hidden transition-all duration-500 group-hover:h-52">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20 pointer-events-none">
           <button
             ...
             className="pointer-events-auto transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 bg-slate-900/50 hover:bg-blue-600 backdrop-blur-sm border border-white/20 text-white rounded-full p-3 shadow-xl"
        ...
        {isCustomizing && (
           <div 
             className="absolute inset-0 bg-slate-950/95 backdrop-blur-md z-40 p-4 flex flex-col animate-fadeIn"
             onClick={(e) => e.stopPropagation()}
           >
             <div className="flex justify-between items-center mb-4">
                <h4 className="text-white font-bold text-sm uppercase tracking-wide">{labels.customize}</h4>
                <button onClick={toggleCustomization} className="text-slate-400 hover:text-white">
             ...
             <div className="flex gap-1 border-b border-slate-800 mb-4">
               <button 
                 ...
                 className={`flex-1 pb-2 text-xs font-bold uppercase transition-all border-b-2 ${
               ...
               {activeCustomTab === 'paint' && (
                 <div className="animate-fadeIn">
                     ...
                     {COLORS.map(c => (
                       <button
                         ...
                         className={`w-8 h-8 rounded-full border-2 transition-all duration-200 transform ${
                           selectedColor === c.id 
                             ? 'border-white scale-110 shadow-lg shadow-white/20 ring-2 ring-blue-500/50' 
                   ...
                   <p className="text-[10px] text-slate-500 text-center mt-4">
                     Select a color to visualize the {bike.model} in a new look.
                   </p>
                 </div>
               )}
             ...
             <button 
               ...
               className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg uppercase tracking-wide shadow-lg shadow-blue-600/20 transition-all mt-4 flex items-center justify-center gap-2"
             >
               <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {isQuickViewOpen && (
        <div 
          className="absolute inset-0 bg-slate-950/95 z-50 p-6 flex flex-col animate-fadeIn"
          onClick={(e) => e.stopPropagation()} 
        >
        ...
        <div className="absolute bottom-4 left-4 bg-black/40 backdrop-blur-sm px-3 py-1 rounded-md border border-white/10 group-hover:bg-blue-600/80 group-hover:border-blue-500/50 transition-all duration-300 z-10">
          <span className="text-xs font-bold tracking-widest uppercase text-white shadow-sm">{bike.brand}</span>
        </div>
      ...
      <div className="p-6 flex flex-col flex-grow relative z-10 bg-slate-900 transition-colors duration-300 group-hover:bg-slate-900/95 border-t border-slate-800">
        <div className="flex justify-between items-start mb-1">
          <h3 className="text-xl font-bold text-white group-hover:text-blue-400 transition-colors duration-300 font-['Exo_2']">
            {bike.model}
          </h3>
          <div className={`text-xs font-bold ${matchScoreColor} bg-slate-800 border border-slate-700 px-2 py-1 rounded shadow-sm group-hover:border-slate-600 transition-colors`}>
            {bike.matchScore}%
          </div>
        </div>
        ...
        <p className="text-slate-400 text-sm line-clamp-2 mb-6 flex-grow leading-relaxed group-hover:text-slate-300 transition-colors">
          {bike.description}
        </p>
        
        {/* Specs Grid */}
        <div className="grid grid-cols-3 gap-2 border-t border-slate-800 pt-4 group-hover:border-slate-700 transition-colors">
          <div className="text-center p-2 rounded bg-slate-800/50 group-hover:bg-slate-800 transition-colors duration-300">
            <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">{labels.engine}</div>
            <div className="text-slate-200 font-semibold text-sm">{bike.specs.engineCC}</div>
          </div>
          <div className="text-center p-2 rounded bg-slate-800/50 group-hover:bg-slate-800 transition-colors duration-300">
            <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">{labels.mileage}</div>
            <div className="text-emerald-400 font-semibold text-sm">{bike.specs.mileage}</div>
          </div>
          <div className="text-center p-2 rounded bg-slate-800/50 group-hover:bg-slate-800 transition-colors duration-300">
            <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">{labels.power}</div>
        ...
        <div className="mt-4 pt-4 border-t border-slate-800 group-hover:border-slate-700 transition-colors">
          <button 
            ...
            className="flex items-center justify-between w-full text-xs font-bold text-slate-500 group-hover:text-blue-400 transition-colors uppercase tracking-wide"
          >
          ...
          >
             <div className="bg-slate-800/80 p-3 rounded-lg border border-slate-700/50 space-y-3">
                <div>
                  <div className="text-[10px] uppercase text-slate-400 font-bold mb-1">{labels.regionalFit}</div>
                  <p className="text-xs text-slate-200 italic leading-relaxed">"{bike.stateFitReason}"</p>
                </div>
                <div>
                   <div className="flex justify-between items-end mb-1">
                      <div className="text-[10px] uppercase text-slate-400 font-bold">{labels.compatibilityScore}</div>
          ...
          {!isExpanded && (
             <div className="mt-2 text-xs text-slate-400 line-clamp-1 italic">
                "{bike.stateFitReason}"
             </div>
          )}
        </div>
